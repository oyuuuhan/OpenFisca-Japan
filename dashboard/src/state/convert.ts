import { FrontendHousehold } from '../state';
...
export const toFrontendHousehold = ({
  context,
}: {
  context: QuestionStateContext;
}): FrontendHousehold => {
  const prefecture = context.寝泊まりしている地域.あなた[0].prefecure;
  const municipality = context.寝泊まりしている地域.あなた[0].municipality;
  ...
  if (
    context['困りごとはありますか？'].あなた[0].selection.includes(
      '仕事について'
    )
  ) {
  ...
  if (
    ...
    if (prefecture === '埼玉県' && municipality === '富士見市') {
